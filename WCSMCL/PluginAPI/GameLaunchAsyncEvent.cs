...
using WCSMCL.Views;
using JavaToolkit = WCSMCL.Modules.Toolkits.JavaToolkit;
    ...
    public class GameLaunchAsyncEvent : Event, ICancellable
    {
        ...
        public override bool Do()
        {
            ...
            if (!IsCanceled)
            {
                ...
                {
                    MainWindow.ShowInfoBarAsync("错误：", $"未选择任何游戏账户！", InfoBarSeverity.Error);
                    return false;
                }

                if (string.IsNullOrEmpty(App.Data.SelectedUser.UserName) || string.IsNullOrEmpty(App.Data.SelectedUser.UserUuid))
                {
                    MainWindow.ShowInfoBarAsync("错误：", $"选择的游戏账户里有信息为空！", InfoBarSeverity.Error);
                    return false;
                }
