...
namespace WCSMCL.Views
{
    public partial class SettingView : Page
    {
        ...
        {
            FrameView.Navigate((Page)FrameView.Content, e.IsSettingsInvoked ? typeof(GameSettingView) : (Type.GetType($"WCSMCL.Views.{((NavigationViewItem)e.InvokedItemContainer).Tag ??= string.Empty}")) ?? typeof(GameSettingView), null, null);
        }
    }
}
