...
namespace WCSMCL.Views
{
    public partial class BlessingView : Page
    {
        ...
        {
            FrameView.Navigate((Type.GetType($"WCSMCL.Views.DownView")) ?? typeof(NewsView));
        }
        ...
        {
            FrameView.Navigate((Type.GetType($"WCSMCL.Views.{((NavigationViewItem)e.InvokedItemContainer).Tag ??= string.Empty}")) ?? typeof(NewsView));
        }
    }
}
